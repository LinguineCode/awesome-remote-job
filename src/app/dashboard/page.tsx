...
export default function DashboardPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/results?limit=10").then((r) => r.json()),
    ])
      .then(([searchData, resultData]) => {
        setSearches(Array.isArray(searchData) ? searchData : []);
        setRecentResults(Array.isArray(resultData) ? resultData : []);
      })
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  {search.year_min && (
                    <span className="text-xs text-gray-500">
                      {search.year_min}
                      {search.year_max ? `–${search.year_max}` : "+"}
                    </span>
                  )}
                  {search.price_max && (
                    <span className="text-xs text-gray-500">
                      Under ${search.price_max.toLocaleString()}
                    </span>
                  )}
                </div>
                {search.ai_notes && (
        ...
        {recentResults.length === 0 ? (
          <div className="rounded-xl border border-dashed border-gray-300 bg-white p-8 text-center">
            <p className="text-gray-500">
              No matches yet. Results will appear here after the daily scan.
            </p>
          </div>
        ) : (
